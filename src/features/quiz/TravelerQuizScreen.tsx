...
export function TravelerQuizScreen({
  ...
  // Icon Resolvers for semantic differentiation
  const getIntentIcon = (value: CurrentIntent) => {
    switch (value) {
      case "RECHARGE":
        return <RechargeIcon />;
      case "NATURE":
        return <NatureLeafIcon />;
      case "NOVELTY":
        return <NoveltyCompassIcon />;
      case "REFLECTION":
        return <ReflectionMoonIcon />;
      case "ACTIVE":
        return <ActiveHikingIcon />;
      case "SOCIAL":
        return <SocialHeartIcon />;
    }
  };

  const getActivityIcon = (value: PreferredActivity) => {
    switch (value) {
      case "NATURE_SCENERY":
        return <SceneryMountainIcon />;
      case "MINDFULNESS_RELAXATION":
        return <LotusMindfulnessIcon />;
      case "LOCAL_CULTURE":
        return <CultureTempleIcon />;
      case "CREATIVE_WORKSHOP":
        return <CreativeBrushIcon />;
      case "LIGHT_EXPLORATION":
        return <ExplorationMapIcon />;
      case "OUTDOOR_ACTIVE":
        return <OutdoorSunIcon />;
    }
  };

  const getBudgetIcon = (value: BudgetBand) => {
    switch (value) {
      case "UP_TO_200K":
        return <WalletTier1Icon />;
      case "AROUND_200_300K":
        return <WalletTier2Icon />;
      case "AROUND_300_500K":
        return <WalletTier3Icon />;
      case "ABOVE_500K":
        return <WalletTier4Icon />;
    }
  };

  const getDurationIcon = (value: DurationPreference) => {
    switch (value) {
      case "HALF_DAY":
        return <ClockHalfDayIcon />;
      case "FULL_DAY":
        return <SunFullDayIcon />;
      case "TWO_D_ONE_N":
        return <MoonCampIcon />;
      case "THREE_D_TWO_N_PLUS":
        return <JourneyCalendarIcon />;
    }
  };

  const getDepartureIcon = (value: DepartureAreaId) => {
    switch (value) {
      case "MALANG":
      case "SURABAYA":
        return <MapPinCityIcon />;
      case "OTHER":
        return <OtherLocationIcon />;
    }
  };

  const getGroupTypeIcon = (value: GroupType) => {
    switch (value) {
      case "SOLO":
        return <SoloUserIcon />;
      case "PARTNER":
        return <PartnerUsersIcon />;
      case "FRIENDS":
        return <FriendsGroupIcon />;
      case "FAMILY":
        return <FamilyHomeIcon />;
    }
  };
  ...
  return (
    <div className="quiz-screen">
      ...
      <div className="quiz-screen__backdrop" aria-hidden="true">
        <img
          src={LOGIN_ATMOSPHERE_VISUAL.svgDataUri}
          alt=""
          className="quiz-screen__backdrop-image"
          loading="eager"
          width="1000"
          height="800"
        />
        <div className="quiz-screen__backdrop-scrim" />
        <div className="quiz-screen__backdrop-grain" />
      </div>

      {/* Floating Viewport Container */}
      <div className="quiz-screen__container">
        {isLoading ? (
          <main className="quiz-card quiz-card--loading">
            <div className="quiz-loading-spinner" aria-hidden="true" />
            <p className="quiz-loading-text">Menyiapkan preferensimu...</p>
          </main>
        ) : (
          <main className="quiz-card" aria-label="Kuis Preferensi JedaIn">
